...
export class Controller extends EventTarget {
  ...
  execute(command,now=performance.now()) {if(this.busy)throw Error('Finish or skip the animation before moving another card.');const transaction=this.model.dispatch(command);this.clock.start(planTransition(transaction),now);this.render();return transaction;}
  frame(now){if(!this.clock.playing)return;this.clock.sample(now);this.render();}
  load(input){this.clock.finish();this.clock.plan=null;this.model.load(input);this.render();}
  ...
  drop(id,side,target,index,now=performance.now()) {
    const source=['left','right'].find(s=>this.model.state[s].some(t=>t.id===id));
    if(!source)throw Error('Unknown card.');
    if(side!==source)return this.execute({type:'move',id,side},now);
    if(target&&target!==id)return this.execute({type:'combine',id,target},now);
    if(Number.isInteger(index))return this.execute({type:'reorder',id,side,index},now);
